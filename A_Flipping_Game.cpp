...
void solve()
{
    ...
    rep(i, 1, n)
    {
        // flipping segment not started

        dp[i][0] = 0;

        // flipping segment started

        dp[i][1] = max(dp[i - 1][1], dp[i - 1][0]) + gain[i];

        // flipping segment ended

        dp[i][2] = max(dp[i - 1][1] + gain[i], dp[i - 1][2]);
    }

    int maxgain = max(dp[n - 1][1], dp[n - 1][2]);

    cout << cnt1 + maxgain << endl;
}
